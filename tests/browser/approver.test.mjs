...
try {
  ...
  const credential = await page.evaluate(async rpId => (await navigator.credentials.create({publicKey: {
    challenge: crypto.getRandomValues(new Uint8Array(32)), rp: {id: rpId, name: 'Isolated synthetic RP test'},
    user: {id: new Uint8Array([1]), name: 'test', displayName: 'Test'}, pubKeyCredParams: [{type: 'public-key', alg: -7}],
    authenticatorSelection: {authenticatorAttachment: 'platform', userVerification: 'required'}}})).toJSON(), rpId);
  ...
  const request = {id: crypto.randomUUID(), origin, page: approvalPage, rpId, targetName: 'Synthetic target', expires: Date.now() + 15000,
    options: {rpId, challenge: crypto.randomBytes(32).toString('base64url'), userVerification: 'required',
  ...
  await page.locator('[data-remote-fido-approval]').waitFor(); await page.keyboard.press('Enter');
  ...
  await page.locator('[data-remote-fido-approval]').waitFor();
