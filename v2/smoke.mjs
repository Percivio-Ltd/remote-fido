...
assert.ok(['google', 'apple', 'openai'].includes(provider));
const raw = JSON.stringify({rpId: `${provider}.com`, challenge: crypto.createHash('sha256').update(`remote-fido-smoke:${nonce}`).digest('base64url'),
  timeout: 60000, userVerification: 'required', extensions: {remoteDesktopClientOverride: {origin: {apple: 'https://idmsa.apple.com', google: 'https://accounts.google.com', openai: 'https://auth.openai.com'}[provider], sameOriginWithAncestors: true}}});
if (role === 'target') {
